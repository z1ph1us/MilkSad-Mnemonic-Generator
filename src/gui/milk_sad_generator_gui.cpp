...
#include <QFontDatabase> // For adding application fonts

// Include your custom headers
#include "MainWindow.h" // Now includes MainWindow class declaration
#include "WorkerThread.h" // Now includes WorkerThread class declaration
...
std::vector<std::string> load_wordlist(const std::string& filename) {
    ...
    if (!file) {
        throw std::runtime_error("Could not open wordlist file at " + WORDLIST_DIR + filename);
    }
...
void save_progress(const std::string& progress_file, uint32_t last_timestamp) {
    ...
    if (out.is_open()) {
        ...
        throw std::runtime_error("Could not write to progress file " + progress_file);
    }
}
...
uint32_t load_progress(const std::string& progress_file) {
    ...
    if (in.is_open()) {
        ...
        if (!in.good()) {
            return 0; // File exists but corrupted/empty
        }
    }
    else {
        throw std::runtime_error("Could not read from progress file " + progress_file);
    }
...
std::tm parse_datetime(const std::string& datetime_str) {
    ...
    if (ss.fail()) {
        throw std::runtime_error("Invalid date/time format. UseYYYY-MM-DD HH:MM:SS");
    }
    return tm;
}
...
void parse_date_range(const std::string& range_str, std::tm& start_tm, std::tm& end_tm) {
    ...
    if (colon_pos == std::string::npos) {
        throw std::runtime_error("Invalid format. UseYYYY-MM-DD:YYYY-MM-DD");
    }
...
uint32_t get_unix_timestamp(int year, int month, int day, int hour, int minute, int second) {
    ...
    #ifdef _WIN32
        _putenv("TZ=UTC"); // Use _putenv on Windows
        _tzset();
    #else
        setenv("TZ", "UTC", 1); // Use setenv on Linux/macOS
        tzset();
    #endif

    std::time_t timestamp = std::mktime(&t);
    if (timestamp == -1) {
        throw std::runtime_error("Invalid date/time for Unix timestamp conversion (mktime failed).");
    }
...
int main(int argc, char *argv[]) {
    ...
    if (gui_mode) {
        ...
        // --- Set global application font to Open Sans ---
        QFont openSansFont("Open Sans");
        // You can set a default size if desired, e.g., openSansFont.setPointSize(10);
        app.setFont(openSansFont);
        // If Open Sans is not found, Qt will fall back to a generic sans-serif font.
        // For embedding the font, you would add this:
        // int fontId = QFontDatabase::addApplicationFont(":/fonts/OpenSans-Regular.ttf"); // Path in your .qrc file
        // QString family = QFontDatabase::applicationFontFamilies(fontId).at(0);
        // QFont openSansFont(family);
        // app.setFont(openSansFont);
        // (This assumes you have a 'fonts' folder in your resources.qrc and the font file there)
        ...
        std::cout << "Milk Sad Generator" << std::endl;
        std::cout << "------------------------" << std::endl;
        ...
        if (mode_choice == 1) {
            ...
            std::cout << "Enter the date range in formatYYYY-MM-DD:YYYY-MM-DD (Start:End): ";
            ...
            uint32_t current_timestamp; // Declared outside the loop
            current_timestamp = load_progress(PROGRESS_FILE_RANGE);
            ...
            for (; current_timestamp <= end_timestamp; ++current_timestamp) {
                if (stop_flag) {
                    std::cout << "\nReceived interrupt signal, stopping generation..." << std::endl;
                    break;
                }
            ...
            if (current_timestamp > end_timestamp) { // Only delete if generation completed
                unlink(PROGRESS_FILE_RANGE.c_str());
                std::cout << "Generation complete. All mnemonics saved to " << output_file << "\n";
            } else {
                std::cout << "Generation stopped. Progress saved to " << PROGRESS_FILE_RANGE << ".\n";
            }
        }
            ...
            uint32_t current_timestamp_loop; // Declared outside the loop
            for (current_timestamp_loop = start_timestamp; current_timestamp_loop <= end_timestamp; ++current_timestamp_loop) {
                if (stop_flag) {
                    std::cout << "\nReceived interrupt signal, stopping generation..." << std::endl;
                    break;
                }
                outfile << generate_mnemonic_bip39(current_timestamp_loop, wordlist) << "\n";
                ...
                if (current_timestamp_loop % REPORT_INTERVAL == 0) {
                    double progress = static_cast<double>(current_timestamp_loop - start_timestamp) / (static_cast<double>(end_timestamp - start_timestamp));
                    std::cout << "Progress: " << std::fixed << std::setprecision(2) << (progress * 100.0) << "% complete\n";
                }
            }

            // Reverting this check to the simpler, original form:
            if (current_timestamp_loop == end_timestamp + 1) { // Check if loop completed naturally
                 unlink(PROGRESS_FILE_FULL.c_str());
                 std::cout << "Generation complete. All mnemonics saved to " << output_file << "\n";
            } else { // Loop was interrupted by stop_flag
                 std::cout << "Generation stopped. Progress saved to " << PROGRESS_FILE_FULL << ".\n";
            }
        }
